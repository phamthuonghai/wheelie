...
class AttentionVisualizer(object):
    ...
    def decode_list(self, integers, decoder='targets'):
        """List of ints to list of str."""
        integers = list(np.squeeze(integers))
        return self.encoders[decoder].decode_list(integers)
    ...
    def get_vis_data_from_string(self, sess, input_string):
        ...
        input_list = self.decode_list(encoded_inputs, 'inputs')
...
def build_model(hparams_set, model_name, data_dir, problem_name, beam_size=1):
  ...
  """
    ...
    if 'dep_parse' in model_name:
        target_dephead = tf.placeholder(tf.int32, shape=(1, None, 1, 1), name='target_dephead')
        translate_model({
            'inputs': inputs,
            'target_dephead': target_dephead,
            'targets': targets,
        })
    else:
        translate_model({
    ...
    att_mats = get_att_mats(translate_model, model_name)
