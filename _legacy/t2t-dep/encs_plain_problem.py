...
class TranslateEncsPlain(translate_encs.TranslateEncsWmt32k):

    def generator(self, data_dir, tmp_dir, train):

        datasets = data_utils.ENCS_PLAIN_TRAIN_DATASETS if train else data_utils.ENCS_PLAIN_TEST_DATASETS
        tag = "train" if train else "dev"
        data_path = translate.compile_data(tmp_dir, datasets,
                                           "encs_plain_%s" % tag)

        vocab_datasets = [
            [item[0], ["encs_plain_%s.lang1" % tag, "encs_plain_%s.lang2" % tag]]
